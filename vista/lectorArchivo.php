...
if(isset($_GET['numOrden'])){
  ...
  $ruta= "../assest/dist/files/manual/".$archivo;
}
...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      // Search text in PDF
      const searchText = (page, keyword) => { return page.getTextContent().then((textContent) => { const textString = textContent.items .map((item) => item.str) .join(' '); return textString.includes(keyword); }); };

      const searchPDF = (keyword) => { const numPages = initialState.pdfDoc.numPages; for (let i = 1; i <= numPages; i++) { initialState.pdfDoc.getPage(i).then((page) => { searchText(page, keyword).then((found) => { if (found) { initialState.currentPage = i; currentPage.value = initialState.currentPage; renderPage(); } }); }); } };

      searchButton.addEventListener('click', () => { const keyword = searchInput.value; if (keyword) { searchPDF(keyword); } });
    </script>
